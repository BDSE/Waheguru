...
myApp.controller('myController', ['$scope', '$timeout', '$filter', '$http', function ($scope, $timeout, $filter, $http) {
	...
	$scope.testArr = [[2, 3, 4], [4, 5, 6], [7, 8, 9]];
	$scope.testFn = function (e) {
		alert(e);
	}
	...
	setTimeout(function () {
		...
		//			$scope.name = 'NameFromAsynTimeout...$appy method';
		//			$scope.handleInput1 = 'setTimeoutDone';
		//		})
		$scope.name = 'NameFromAsynTimeout...$digest method';
		$scope.handleInput1 = 'setTimetOutDoneLiveReloadTest';
		$scope.$digest();
