 var getOverlayContainer = function() {
    ...
    var overlayDirectiveApp = angular.module('overlayDirectiveApp', []);
    ...
    overlayDirectiveApp.provider('wcOverlayConfig', function(){
    ...
    overlayDirectiveApp.factory('httpInterceptor', function () {
        return {}
    });

    //Hook httpInterceptor factory into the $httpProvider interceptors so that we can monitor XHR calls
    overlayDirectiveApp.config(['$httpProvider', function ($httpProvider) {
        $httpProvider.interceptors.push('httpInterceptor');
    }]);

    //expose overlay show / hide functionality
    overlayDirectiveApp.service('overlayDisplayService', ['$window', function($window){
    ...
    overlayDirectiveApp.directive('overlay', ['$q', '$timeout',  'httpInterceptor','wcOverlayConfig', 'overlayDisplayService',  function ($q, $timeout, httpInterceptor, wcOverlayConfig, overlayDisplayService) {
        var link =  function (scope, element, attrs) {
                ...
                function processRequest() {
                    ...
                    if (queue.length == 1) {
                        ...
                        }, scope.overlayDelay ? scope.overlayDelay : overlayConfig.delay); //Delay showing for 500 millis to avoid flicker
                    }
                }
                ...
                function processResponse() {
                    ...
                    if (queue.length == 0) {
                        ...
                        }, scope.overlayDelay ? scope.overlayDelay : overlayConfig.delay);
                    }
                }
