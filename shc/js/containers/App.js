...
class App extends Component {
    ...
    render() {
        const { isChanging, isFetching, showSpinner } = this.props;
        if(this.state.invalidData){
            ...
            return (
                <div className="main dashboard">
                    <div className={ "overlay loader col-mod-12" + (isChanging || isFetching || showSpinner ? " showing" : "") }></div>
                    <div className={ "spinner-wrap" + (!isChanging && !isFetching && !showSpinner ? " display-none" : "") }>
...
const mapStateToProps = (state) => {
    const { getData, getState, modal, spinner } = state;
    const {
        mode,
        submode,
        params,
